...
class Paginator2:
    ...
    def is_fitting(self) -> bool:
        """Return True if all pages can be displayed at once."""
        return self.num_pages < self._pages_in_block - 1
    ...
    def _iterate_short(self) \
        ...
        Version, where all pages are displayed.
        """
        gen = range(1, self.num_pages + 1)
        for i, value in zip(gen, self._sequence):
            yield {
                'is_dummy': False,
                'is_current': i == self._current_page,
                'number': i,
    ...
    #                 'value': x,
    #             }
    #             for i, x in enumerate(self._sequence, start=1)
    #         ]
    #
    #     position = self.current_page
    #

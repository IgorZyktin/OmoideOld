...
def random_records(query: Query, index: Index) -> SearchResult:
    ...
    if query.themes is not None and query.themes:
    ...
    return SearchResult(
        items=paginate(
            sequence=[x.dict() for x in chosen_records if x is not None],
            page=query.page,
...
def specific_records(query: Query, index: Index) -> SearchResult:
    ...
    chosen_records = [index.by_uuid.get(x) for x in target_uuids]
    ...
    # TODO - add pages and flag that indicates that sequence is not ended here
    return SearchResult(
        items=paginate(
            sequence=[x.dict() for x in chosen_records if x is not None],
            page=query.page,
